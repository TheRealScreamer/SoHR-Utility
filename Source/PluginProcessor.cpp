...
void SoHRUtilityAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    bool currentPhaseState = getPhaseState();
    ...
    {
        //makes input knob work
        buffer.applyGain(channel, 0, buffer.getNumSamples(), currentInputGain);
    ...
    for (int channel = 0; channel < buffer.getNumChannels(); channel++)
    {
        //makes output knob work
        buffer.applyGain(channel, 0, buffer.getNumSamples(), currentTrimGain);
    }
}
